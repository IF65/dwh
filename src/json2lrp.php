...
$debug = false;
if ($options->getOption('debug') != null) {
	$debug = true;
}
...
// -----------------------------------------------------------
	...
	/** da tenere solo in fase di test */
    ...
    if ($debug) {
	    file_put_contents($config->getSetup($config::EXPORTFOLDER) . DIRECTORY_SEPARATOR . 'tap.txt', implode("\n", $tapMancanti));

    	$fileName = "DC20$anno$mese$giorno" . '0' . $store . '001.DAT';
    	file_put_contents($config->getSetup($config::EXPORTFOLDER) . DIRECTORY_SEPARATOR . $fileName, implode("\n", $righe));

	    $fileName = "DC20$anno$mese$giorno" . '0' . $store . '001.CTL';
	    file_put_contents($config->getSetup($config::EXPORTFOLDER) . DIRECTORY_SEPARATOR . $fileName, '');
    }
